...
#   dist/magpie-<version>.apk  developer-signed universal APK: GitHub release,
#                                F-Droid reproducible verification, sideload
#   dist/magpie-<version>.aab  upload-key-signed bundle for Play Console
#
# Never run by CI. The keystore lives outside the repo and its password stays
# in the system keyring (secret-tool lookup service magpie-keystore key
...
KEYSTORE="${MAGPIE_KEYSTORE:-$HOME/keys/magpie-upload.jks}"
ALIAS=magpie-upload
...
KSPW=$(secret-tool lookup service magpie-keystore key upload) || {
  echo "no keystore password in the keyring (service magpie-keystore key upload)"; exit 1;
}
export KSPW
...
APK_OUT="dist/magpie-$VERSION.apk"
AAB_OUT="dist/magpie-$VERSION.aab"
...
# releases/latest/download/magpie.apk across versions.
cp "$APK_OUT" dist/magpie.apk
